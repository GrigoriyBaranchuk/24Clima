...
**Новые задачи от владельца (2026-08-23), в работу не брались**

1. **Переключатель языка — перенести влево.** Сейчас первый элемент правой
   actions-зоны. Загвоздка: `HeaderShell` пакета даёт четыре слота
   (logo/nav/actions/mobileMenu), левого слота под язык нет — либо класть
   внутрь `logo`, либо расширять пакет (PR → тег → bump в двух проектах).
   Отдельно решить мобильную строку, где переключатель рядом с бургером.
2. **Кнопку «Tienda» сделать заметнее (CTR).** Сейчас нейтральная outline-
   кнопка по решению 3A. Упирается в инвариант «WhatsApp — единственный
   зелёный заливкой»: усиление магазина не должно съесть основной лид-CTA,
   поэтому это решение владельца о приоритете «покупка против WhatsApp-
   лида», а не просто визуальная правка.

   Детали и варианты — в `concepts/header-navigation`, раздел
   «Запрошено владельцем».

---

## Сессия 2026-08-23 — Новые услуги: gypsum, скрытый климат (ductos), магазин VIP Aire, сметы B2B

**Что взяли.** Владелец поставил задачу: собрать фундамент под новые услуги,
исследовать спрос в Панаме, определить стратегию «стать главным подрядчиком»,
подготовить товары партнёра к листингу в `/tienda`. Источники — Notion-каталог
VIP Aire и eurocalidad.lat. Роли по решению владельца: Fable дирижирует, код
пишет Opus, решения проверяет Codex; общение — простым языком.

**Что выяснили (DataForSEO + 7 исследовательских агентов Opus, ~300 KB сырья
в `memory/raw/research/`).** Гипсокартон — 5 000+ запросов/мес, рынок без
лидера (0 подрядчиков в Google Maps, выдача — Instagram), цены Contraloría
$19–54/м². Канальные системы — узко ($170 `fan coil`, CPC до $2,85), ярус
design-build — 5 фирм с <10 отзывами, никто не публикует цены. Связку
«ducted + gypsum» как один продукт не продаёт никто. VIP Aire — VIP AIRE
ACONDICIONADO S.A., дистрибьютор Twitoplast/Genesis, онлайн отсутствует.
EuroCalidad = SUNNY RAMPAGE INC. = TropiClima. Лицензирование: подписант JTIA
должен быть панамцем; компания регистрируется за B/.500 + PIRE B/.175.
Покупатели 2026 с капексами (Platinum/KFC US$15M, Vierci 105 точек, Pizza Hut
greenfield, Regus). Закупки: MIPYME +5%, фианзы 50% obras, удержание 50% ITBMS.

**Решения владельца.** Четыре направления (инспекции — нет); две отдельные
услуги на сайте (Gypsum; Clima oculto llave en mano); потолки + канальные
первыми; бригада своя + найм под присмотром EuroCalidad; EuroCalidad на сайте
не показывать, видео из Германии — с подписью; стартуем без лицензии, **будет
человек с правом подписи**; цены: gypsum liso desde $35/m² (под покраску,
покраска отдельно), LED-короб desde $25/ml, стены и дизайн — cotización,
canalные — cotización gratis; магазин: наценка 35% + ITBMS 7% на все четыре
прайса VIP Aire (`~/Downloads/VIP_AIRE`), медь по метрам от бухты 45 м, мин.
отрез 5 м; решётки с фильтром от $40 поштучно; люки — премиум через факты.

**Ревью Codex (сессия 01a02fda).** Принято: убрать $55 «con diseño», LED —
отдельной строкой; $35 с условиями desde; порядок магазина медь → базы →
решётки; гарантия и тех.ограничения — в пресейл; RAV не использовать до
чтения; KPI — лиды. Не принято: отказ от единой наценки; откладывание clima
oculto. Стратегия v2 — `memory/inbox/2026-08-23-strategy-draft.md` (локально).

**Процессное.** Три из четырёх агентов упали по лимиту сессии (сброс 3:40),
возобновлены через SendMessage с запретом субагентов — дописали. WebSearch-
бюджет (200) выгорел у всех — добирали через DataForSEO SERP и прямые fetch.
Защита worktree блокирует составные shell-команды — Codex запускали через
субагента и скрипт в `$CLAUDE_JOB_DIR/tmp`.

**Wiki.** Новые: `sources/2026-08-23-panama-new-services-research`,
`concepts/new-services-ducted-gypsum`, `concepts/panama-contractor-licensing`,
`entities/vip-aire`, `entities/eurocalidad`; обратные ссылки в 6 страницах;
...
**Закрытие (2026-08-25).** PR #51 смержен в main (squash `2fc4258`) по «ок
мерж» владельца; перед мержем в ветку влит origin/main (конфликт нумерации
грабель: skew остался №31, biome стал №32). Прод проверен: новый слаг 200
c 8 FAQ и галереей, gypsum с галереей и видео-фасадом (iframe только по
клику), старый слаг 404 (не индексировался), sitemap и CSP корректны.
...
- Контент: 5 статей, первая — «¿Cuánto cuesta un cielo raso de gypsum en
  Panamá?»; статья-хаб «¿Qué es un fan coil?» (170/мес, CTA на услугу
  por-ductos — решение Codex 01a03212); ссылки в футере.
  (Галерея фото — СДЕЛАНА 2026-08-24, ServiceGallery в PR #51.)
- UI-мелочь: нижняя ссылка-заголовок карточки ductos на /servicios/
  переполняет карточку (длинный H1 обрезается по краям) — перенос строки
  вместо клипа. Замечено 2026-08-25, владельцу предложен фикс.
...
- Переписать статью с «$4 500–5 500 за 150 m²» (реальный коридор $9–20k).
- Прочитать RAV (Res. 117/2013) прежде чем ссылаться на него.
...
## Сессия 2026-08-25/26 — /seo-tasks: id 27 + id 29, фикс скролла навигации, фикс билда merchant-feed

**Цикл /seo-tasks.** Забраны 3 accepted-рекомендации (id 27, 26, 29).
Порядок согласован с владельцем: 29 → 26; гео-страницы районов в рамках
id 26 — делаем (решение владельца).

**id 27 (gsc_totals STALE) — закрыт без кода.** Диагностика: рекомендация
создана 12.08 01:54 UTC — ДО первого прогона источника gsc_totals (07:01);
с тех пор 14 прогонов ok подряд. Неделя до 09.08 перепроверена (63 клика /
6031 показ подтверждены), следующая — 75 кликов: недельный шум, не регрессия.

**id 29 (страницы у границы топ-10) — выполнен (PR #56, merge 0c063ad).**
План прошёл seo-reviewer (flag-with-conditions) и Codex (4×P1 учтены).
Код 3410bf1: футер — прямые локализованные ссылки /servicios/* вместо якоря
/#servicios (0 прямых ссылок было!); reparacion — блок зон (TARGET_SERVICES),
цена hero «desde $35» (гейт по translationKey!), faq6/faq7, intent-note
reparación↔carga-de-gas/emergencia, секция «¿Qué reparamos?» (новый слот
extraSection в ServiceExpandedContent); limpieza — Costa del Este в
description/intro; instalacion — minisplit (слова не было на странице при
трекаемом ключе). Статья recarga реструктурирована ПРЯМО в Supabase
(articles, force-dynamic = мгновенный деплой, применено 26.08 после «ok»):
настоящие H2/H3 (было 0!), title 95→65 симв. с интентом «cuándo/síntomas»
(ценовой интент отдан второй статье ...cuanto-cuesta... — развод
каннибализации по вердикту обоих ревью; merge+301 пересмотреть по GSC через
4–6 недель), цены $65–145 → $120–210 (business-data), гарантия 60 días,
3 ссылки на /servicios/carga-de-gas (было 0). Бэкапы и скрипт применения —
в tmp джоба. id 29 закрыт в БД с resolution.

**Фикс скролла (репорт владельца, ec3aa6d).** Переход между страницами
открывался на старой позиции скролла: scroll={false} был вшит почти во все
внутренние Link (NavItem шапки, FooterNavLink, списки статей, корзина).
Оставлен только у якорей /#servicios через хелпер isHashNav (src/lib/nav.ts)
с комментариями. Кастомного скролл-кода нет — Next 15.3 сам гасит CSS smooth
при навигации. Грабля №33.

**Фикс билда merchant-feed (a6695f2).** 2 из 3 превью-билдов падали:
route-level ISR (revalidate=3600) пререндерил /merchant-feed.xml на билде,
фид тянул весь каталог из shop-api — холодный старт бэкенда превышал
60-секундный бюджет статик-экспорта. Фикс: revalidate=0 (рендер по запросу);
кэш остался (fetch next.revalidate 3600 + s-maxage=3600). force-dynamic
отклонён Codex'ом: молча выключает Data Cache. Грабля №34.

**Осталось / TODO**
- id 26 (accepted): усилить 4 сервисные страницы под гео + гео-страницы
  районов (решение владельца: делаем) + статья-хаб «no enfría».
- Follow-up: AbortSignal-таймауты в tienda api-client (sitemap.ts — та же
  билд-зависимость от shop-api, деградирует мягко, но без таймаута).
- Позиции 4 URL id 29 проверить в seo_rankings через 1–2 недели;
  каннибализацию recarga-статей — по GSC через 4–6 недель.
- Supabase MCP теперь ВИДИТ проект сайта (read-only); записи — по-прежнему
  REST с service-role. Гранула обновлена в вики.

**Дополнение (конец сессии, 2026-08-26/27).** После мержа PR #56 прод-деплой
упал ТРИЖДЫ: сначала 0c063ad и мерж памяти 0333156 — оба на `/sitemap.xml`
(«more than 60 seconds» ×3 попытки). Выяснилось: graceful-degradation
sitemap (try/catch → skip tienda-URL) не работал, потому что зависший fetch
к холодному shop-api — НЕ ошибка; catch не наступал до убийства воркера по
60-секундному лимиту экспорта. Фикс — PR #58 (55b754f, merge 223e87f):
таймаут 45 с в fetchCatalogCached через Promise.race (НЕ AbortSignal —
signal документированно отключает request-мемоизацию Next, на ней сидят
продуктовые страницы). После мержа прод задеплоился успешно, новый билд
проверен на живом сайте (футер-ссылка /servicios/reparacion в HTML,
dpl_EhNyDZ…). TODO «таймауты api-client» закрыт этой же сессией.
Идея на будущее (не в работе): держать shop-api тёплым внешним пингом —
уберёт и 45-секундные ожидания на билде, и 503 первого запроса фида.

---

## Сессия 2026-08-26 (вечер): диагноз «Algo salió mal» после деплоя — это 504 /tienda, не skew

**Контекст.** Владелец снова увидел экран «Algo salió mal» «после новой
версии сайта» (скриншот без строки `Ref:`). Прошлый такой инцидент
(2026-08-23) был skew (грабля №31), поэтому первая гипотеза была та же.

**Диагноз (по Vercel runtime-логам).** Это НЕ skew. Runtime-группа
«Task timed out after 300 seconds»: 15 случаев, 12 пользователей за
28.06–26.08; шесть 504 на `/tienda/` только за 26.08 (14:51–21:47 UTC),
все на старом билде dpl_Gs7LqS…. Механизм — грабля №34 в runtime-виде:
`/tienda` — force-dynamic, каталог фетчится на каждый запрос; холодный
shop-api на Render вешает fetch навсегда, try/catch в getProductsSafe
бессилен (промис не решается), функция висит до лимита 300 с → 504 → на
клиенте error boundary без digest. Отличительный признак от skew: НЕТ
строки `Ref:` на экране.

**Статус.** PR #58 (таймаут 45 с в fetchCatalogCached) закрыл и этот
runtime-путь — задеплоен в прод 26.08 ~23:23 UTC. Проверка после деплоя:
`/tienda/` — 200 за ~1.1 с, каталог рендерится. Новых 504 быть не должно;
худший случай теперь — 45 с ожидания + пустой каталог (home/category)
или error screen с `Ref:` (product, осознанно).
...
## Сессия 2026-08-26/27 (ночь): разведка «портативные кондиционеры» — продажа + аренда + подмена

**Контекст.** Владелец: нужно быстро вывести магазин на оборот $10k/мес.
Идея — портативные напольные кондиционеры: продавать, сдавать в аренду и
оставлять как подменные при затянувшемся ремонте («no te dejamos en el
calor»). Сессия — чистая разведка, кода нет.

**Что выяснили (4 сырых отчёта в `memory/raw/research/2026-08-26-*`,
вики-страница `concepts/portable-ac-business.md`).**
- Спрос есть и растёт: «aire acondicionado portatil» 2 900/мес avg по
  Панаме, +132% г/г, пик апр–июл (до 6 600), низ окт–ноя. Ads почти не
  выкуплен (CPC $0.17–0.36, LOW). Аренда через поиск не продаётся
  (~10 запросов/мес) — но конкурентов по аренде в стране ноль.
- Розница: $200–270 за 12k BTU (Hisense везде); friopanama.com — прецедент
  HVAC-магазина в топ-5 SERP.
- Поставщик «Русклимат» (идея владельца): холдинг реальный, завод в
  Чжуншане есть, но мобильники — OEM Gree/Midea/TCL, 60Гц-версий нет,
  экспорта вне СНГ нет, платёж в РФ-юрлицо — комплаенс-риск. Вердикт:
  слабый вариант, держать вторым треком против прямых ODM (JHS и др.,
  9k BTU ~$130–160 FOB при 100+ шт).
- КРИТИЧНО: Панама 120В/60Гц, версии СНГ 220В/50Гц не подходят (другой
  компрессор/мотор) — заказывать только 115В/60Гц.
- Налоги импорта: HS 8415.82 → DAI 15% + ITBMS 7% ≈ +23% от CIF
  (проверено по аранселю ANA).
- Выдана пошаговая инструкция выхода через ZLC без резидентства:
  котировки IMEXCA/Gera/Panafoto B2B → тест-партия 10–20 шт
  (~$243/юнит landed при цене оптовика $190) → продажи+аренда → при
  подтверждении спроса прямой контейнер к пику сезона (заказ окт–ноя →
  приход янв–фев). ZLC — скорость и проверка спроса, маржа — в контейнере.

**Решения.** НЕ приняты — владелец думает. Модель к $10k/мес предложена:
~20 продаж + флот аренды 10–15 шт + B2B/подменные.

**Осталось (если направление запускается).**
- Живые котировки: IMEXCA/Gera/Panafoto B2B (115В/60Гц, 10/20/50 шт);
  параллельно RFQ к JHS/фошаньским ODM и Русклимату (4 вопроса: юрлицо,
  60Гц, MOQ, FOB 50/100/300).
- Таможенный брокер: подтвердить порог B/.500 Carta Paquete и тарифы
  после Res. 352/2024.
- Категория «Aires portátiles» + лендинг аренды на /tienda, Ads-кампания.
- Из прошлых сессий: id 26 (гео-страницы районов) — синергия с этой нишей.

## Сессия 2026-08-27/28 — наклейка на заднее стекло авто + /qr редирект (PR #64)

**Задача:** наклейка на всё заднее стекло Lexus RX350 2010 (авто владельца):
бренд 24clima.com, QR, офферы «кондиционеры с установкой» и «подменный
напольный кондиционер на время ремонта».

**Дизайн (Claude Design canvas, артефакт
https://claude.ai/code/artifact/9f202ab5-3091-42c8-b14f-884c0527a3c9):**
- Полотно 1400×760 px (1 px = 1 мм ≈ стекло 140×76 см), микроперфорированный
  винил one-way vision, лекало снимает типография со стекла.
- Брейншторм концепций с Codex: v1 «всё сразу» продавала слишком много;
  выбрана (user + Codex №1) концепция «подменный аппарат как герой»:
  «No te dejamos pasando calor» / «Te prestamos un aire portátil mientras
  reparamos el tuyo» + WhatsApp-пилюля + QR 34 см (чёрный на белом, EC=H)
  + VENTA·INSTALACIÓN·MANTENIMIENTO·REPARACIÓN под QR. Синергия с нишей
  «портативные кондиционеры» (сессия 26/27.08).
- Логотип: тёмная версия из public/images/logo.svg (звёздочка заменена на
  снежинку по просьбе владельца, «clima» белым, «.com» зелёным).
- Ревью Codex по печати: rgba-плашки исчезают на перфорации (заменена на
  сплошную #0e2036), QR чистым чёрным, тест-образец QR на плёнке до тиража,
  контент вне верхних углов трапеции и полос обогрева.
- Файл для типографии: ~/Downloads/calcomania-24clima-140x76.jpg
  (5600×3040, ~100 dpi при полном размере; headless Chrome + sips).

**Код (PR #64, merge b6e4869…, план ревьюен Codex):**
- `src/app/qr/route.ts` — GET /qr: 302 → wa.me/50768282120 с предзаполненным
  «Hola, vi el QR del auto de 24clima y quiero una cotización»; лог скана
  через `after()` из next/server ПОСЛЕ ответа (не блокирует редирект,
  no-op без env). QR на наклейке кодирует 24clima.com/qr, а не wa.me —
  назначение можно менять после печати + статистика сканов.
- `supabase/migrations/008_qr_scans.sql` — таблица qr_scans (время,
  user-agent, страна, source='car-sticker'; НЕ контакты), RLS без политик.
  Применена владельцем через SQL editor 2026-08-28, проверена по REST.
- `src/middleware.ts` — /qr в negative lookahead матчера (иначе next-intl
  и слэш-нормализация перехватят путь).

**Грабля №35:** `vercel link --yes` привязал папку к пустому
проекту-дубликату «24clima-site» вместо прод-проекта «24-clima» —
в team два проекта на один репо 24Clima. Перепривязано.

**Осталось:**
- Проверить прод после деплоя: скан QR с экрана → WhatsApp; строка в qr_scans.
- Отдать JPG в типографию (лекало со стекла, тест QR на плёнке до тиража).
- Опционально: uptime-мониторинг /qr (рекомендация Codex).

## Сессия 2026-08-27 — консультация: где живёт телеграм-бот постинга товаров

**Вопрос:** бот для постинга товаров в 24clima-shop — отдельный проект или
писался в рамках shop? Стоит ли продолжать апгрейдить его там же?

**Найдено:** бот — часть монорепо 24clima-shop, отдельного репозитория нет.
Код: `apps/shop-api/app/agent/bot/` (handlers, keyboards, album, auth,
notify). История — этапы A–F: a79787e (listing bot + arq + listing_jobs),
a2d3464 (превью/правки/publish), фото-гейт v2/v3, 54e9cbc (Render deploy),
4f2ef4a (PR #11, разнообразие карточек). Деплой — combined agent process
вместе с shop-api на Render.

**Решение (рекомендация принята):** апгрейдить бота дальше внутри монорепо
24clima-shop. Причины: сцеплен с shop-api по SQLAlchemy-моделям
(`app/models/listing.py`), alembic-миграциям, машине состояний listing_jobs,
очереди arq и деплою; вынос = дублирование моделей или новый API-контракт
без добавленной ценности. Пересмотреть только если бот станет отдельным
продуктом (постинг для других магазинов) или понадобится независимое
масштабирование.

## Сессия 2026-09-04 — SEO-агент ложно требовал «чинить DataForSEO» (PR #69)

**Повод:** владелец принёс диалог с SEO-агентом: «модули dfs_rankings /
dfs_onpage / dfs_backlinks STALE/failed, почините синхронизацию DataForSEO,
без неё оцениваем позиции вслепую». Так три недели подряд.

**Диагноз (три независимых источника):**
- Логи GitHub Actions `seo-dataforseo.yml`: каждый понедельник HTTP 200 по
  всем трём источникам (31.08: rankings 12 строк $0.058, onpage 17,
  backlinks 1; итого $0.084 — те самые «подозрительные $0,08»).
- `seo_sync_runs`: 5 недель подряд `ok`, ни одной ошибки.
- `seo_rankings`: по 12 ключам есть замеры за 4 понедельника.
Синк исправен. Ложная тревога — из двух багов отчёта в `seo-aggregate.ts`:
1. `STALE_DAYS = 3` для всех источников, а `dfs_*` ходят раз в неделю → с
   четверга по воскресенье они всегда STALE (разговор был в пятницу).
2. `our_position = null` (сайт вне топ-20) за прошлую неделю рендерился как
   «—», текущий — как «>20»; агент читал «—» как «замера не было».

**Фикс (PR #69, план ревьюен Codex, замечания учтены):**
- `STALE_DAYS_BY_SOURCE` (daily 3 / `dfs_*` 8) + `staleDaysFor()`,
  `RUNS_WINDOW_DAYS = max+1`, стоимость цикла = последний прогон по источнику
  (иначе два понедельника в окне удваивали «$/нед»).
- `prevMeasured` в строках рейтинга, `formatPosition` / `effectivePosition`;
  три рендера (контекст агента, дайджест, дашборд) — `>20` для измеренного
  отсутствия, `—` только без строки; цвета дашборда сравнивают `>20` как 21.
- Подпись cadence/глубины SERP в контексте агента; `partial` (PSI 25/26 URL —
  норма) отдельной строкой, не STALE/failed; on-page читается с недельным
  окном; `seo_rankings` — 16 дней.
- Проверено: tsc/biome/next lint чисто; smoke на живой базе — `dfs_*`
  stale=false (age 4d), `(prev >20)` вместо `(prev —)`.

**Что реально видно в позициях (10.08 → 31.08):** «recarga de gas» 5 → 10,
«precio limpieza» 15 → 22 — плавное снижение по двум коммерческим ключам на
фоне кликов 69 → 60 → 48; «técnico 24 horas» стабильно 5; 5 ключей стабильно
вне топ-20. Грабля №36.

**Осталось:**
- Мерж PR #69 по «ok» владельца; после мержа перечитать отчёт агента на
...
## Сессия 2026-09-17 — блокирующий CSS: токены в globals.css, inlineCss отвергнут (ветка perf-css-requests)

**Повод:** после PR #77 прод Lighthouse mobile — 99, LCP 1,4 с; PSI владельца всё ещё
показывает «запросы, блокирующие отрисовку, 560 мс» — три CSS-файла главной
(tailwind 17,9 KiB, @font-face 2,1 KiB, токены 2,0 KiB). Вопрос владельца: «есть ли
в большом файле CSS, который мы не используем» — ответ: мёртвого нет (Tailwind
выбрасывает всё, чего нет в коде), но для главной лишнее ~85%: `.prose*` 28,6 KB
из 105 KB (только статьи/товары), hover 9 KB, lg: 5 KB.

**Сделано (Codex consult → Opus → проверка Fable):**
- Коммит 562f27d: `@import "@24clima/design/tokens.css"` первой строкой globals.css +
  `postcss-import` в postcss.config.mjs (без него Next CssChunkingPlugin, лимит
  100 KB, оставлял токены отдельным файлом — грабли №40). CSS-запросов 3 → 2,
  разметка главной побайтно та же, smoke 632/632.
- Эксперимент `experimental.inlineCss` (коммит b72e06d, НЕ в ветке): FCP 1369 → 1161,
  но HTML 185 → 405 KB, трафик +65 KB, флаг «не для прода» — отвергнут.
- Замеры (локально, медиана из 3): main 91 / LCP 3332; A 90 / 3480; B 94 / 3072 —
  шум ±3 балла, устойчиво лучше только FCP.

**Осталось:**
- Мерж PR ветки perf-css-requests по «ok»; после деплоя — PSI 2–3 прогона.
- README пакета 24clima-design всё ещё говорит «импорт в layout.tsx» — поправить
  при следующем релизе пакета.
- Возможный шаг 3: вынести `.prose*` в CSS блога (−28 KB с каждой не-блоговой
  страницы), только с визуальной сверкой статей — порядок каскада.
