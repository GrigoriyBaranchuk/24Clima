...
export async function generateMetadata({
  ...
  if (!translationKey) return { title: "Service Not Found" };
  const seoMeta = SERVICE_SEO_META[translationKey];
  const seoTitle = seoMeta?.title.es;
  const seoDesc = seoMeta?.description.es;
  const canonicalUrl = `${base}/servicios/${service}/`;
  const keywords = getServiceKeywords(getSeoKey(translationKey), "es");
  ...
  return {
    ...
    openGraph: {
      ...
      url: canonicalUrl,
      images: [
        { url: imageFullUrl, width: 712, height: 500, alt: seoTitle },
      ],
    },
    twitter: {
...
export default async function ServicePage({
  ...
  const whatsappMessage = encodeURIComponent(`${tWhatsapp("general")} - ${title}`);
      ...
      <main id="main-content" className="pt-20">
            ...
            <div className="grid lg:grid-cols-2 gap-12 items-center">
              <div>
                ...
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <Badge className="bg-white/20 text-white border-white/30">24/7</Badge>
                </div>
                ...
                <div className="flex flex-col sm:flex-row gap-4">
                  <Button asChild size="lg" className="bg-whatsapp-contrast hover:bg-whatsapp-contrast-hover text-white font-semibold">
                  ...
                  {isCleaningPage && (
                    <Button asChild size="lg" className="bg-white text-[#1e3a5f] hover:bg-gray-100 font-semibold">
              ...
              <div className="hidden lg:block">
                <div className="relative h-[400px] rounded-2xl overflow-hidden shadow-2xl">
                  <Image src={imageUrl} alt={title} fill className="object-cover" sizes="(max-width: 1024px) 0px, 50vw" />
        ...
        {!isCleaningPage && (
          <section className="py-16 lg:py-24 section-gradient">
            <div className="container mx-auto px-4 lg:px-8">
              <h2 className="text-2xl sm:text-3xl font-bold text-[#1e3a5f] text-center mb-12">{title}</h2>
        ...
        <section className="py-16 bg-gradient-to-r from-[#1e3a5f] to-[#0d2240]">
          <div className="container mx-auto px-4 lg:px-8 text-center">
            <h2 className="text-2xl sm:text-3xl font-bold text-white mb-4">{t("urgentTitle")}</h2>
            <p className="text-white/80 mb-8 w-full max-w-2xl mx-auto">{t("urgentSubtitle")}</p>
            <Button asChild size="lg" className="bg-whatsapp-contrast hover:bg-whatsapp-contrast-hover text-white font-semibold">
              <TrackedWhatsAppLink href={whatsappLink} eventName="Lead">
                <svg className="w-5 h-5 mr-2" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
        ...
        <ServiceCitations service={service} locale="es" />
                ...
                <Link key={otherSlug} href={`/servicios/${otherSlug}`}>
                  <Card className="card-hover border-0 shadow-lg bg-white h-full">
                    <CardContent className="p-6">
                      ...
                      <h3 className="font-semibold text-[#1e3a5f] mb-2">{t(`${tk}.title`)}</h3>
                      <p className="text-base text-gray-600 line-clamp-2">{t(`${tk}.description`)}</p>
                    </CardContent>
                  </Card>
                </Link>
              ))}
