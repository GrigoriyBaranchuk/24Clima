...
export type ProductVariant = {
  ...
  sort_order: number;
  /**
   * Axis key → value for a product sold along several axes, e.g.
   * `{ diametro: "1/4″ × 3/8″", presentacion: "Rollo 45 m" }`. Absent/null for
   * products with a single axis (then `label_es` is the whole story) and on an
   * older backend — every reader must stay null-safe.
   */
  options?: Record<string, string> | null;
};
/**
 * One axis a product varies by ("Diámetro", "Presentación"), with its values in
 * display order. Present only when the product really has ≥ 1 declared axis;
 * null / absent means "one axis, use variant.label_es" (the old behaviour).
 */
...
export type ProductImage = {
  ...
  sort_order: number;
  /**
   * Backend `product_images.kind`: main, original, official, studio, infographic,
...
export type ProductList = {
  ...
  /** Optional: axes the variants vary by. Null/absent = single axis (variant.label_es). */
  variant_axes?: VariantAxis[] | null;
  /**
   * Who wrote name/description: "ai" (generated by the listing bot — the Merchant
   * feed must then mark the text as `trained_algorithmic_media`) or "human".
