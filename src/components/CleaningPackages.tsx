    ...
    <section id="paquetes" className="py-8 lg:py-20 section-gradient">
      <div className="container mx-auto px-4 lg:px-8">
        ...
        <div className="w-full max-w-2xl mx-auto text-center mb-6 lg:mb-10">
          <h2
            className="text-xl lg:text-4xl font-semibold text-[#1e3a5f] mb-2 lg:mb-3"
            style={{ letterSpacing: "-0.2px" }}
          >
            {t("title")}
          </h2>
          <p className="text-[13px] lg:text-base text-gray-600">
            {t("subtitle")}
          </p>
        </div>
        ...
        <div
          className="w-full flex gap-3 overflow-x-auto snap-x snap-mandatory pb-2 lg:pb-0 lg:grid lg:grid-cols-3 lg:gap-6 lg:overflow-visible max-w-5xl mx-auto"
          style={{ scrollbarWidth: "none", WebkitOverflowScrolling: "touch" }}
        >
          {packages.map((pkg) => (
            <Card
              key={pkg.key}
              className={`relative overflow-hidden transition-[transform,box-shadow] duration-300 hover:shadow-xl min-w-[75vw] lg:min-w-0 snap-center ${pkg.bgClass} ${pkg.borderClass} ${
            ...
            >
              ...
              <CardHeader className="text-center pb-2">
                ...
                >
                  <pkg.icon
                    className={`w-6 h-6 ${pkg.highlight ? "text-white" : "text-[#1e3a5f]"}`}
                  />
                </div>
                ...
                >
                  <a
                    href={getWhatsAppLink(
                      t("whatsappOrderMessage", { name: pkg.name }),
                    )}
                    target="_blank"
