        ...
        >
          {
            await Promise.all(
              STATS.map(async (stat, index) => {
                ...
                return (
                  // as="li" so the animation wrapper *is* the list item: with a
                  // <div> in between, the <li> stopped being a direct child of
                  // the <ul> on desktop and the list lost its semantics.
                  <RevealOnDesktop
                    key={stat.labelKey}
                    as="li"
