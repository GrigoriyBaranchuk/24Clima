...
Закрыто по циклу 2026-08-25/26: **id 27** (gsc_totals STALE — источник
здоров, рекомендация была создана до его первого прогона; без кода) и
**id 29** (4 URL у границы топ-10 — PR #56, merge 0c063ad + статья recarga
реструктурирована прямо в Supabase). В accepted остался **id 26**
(коммерческие запросы вне топ-20; решение владельца: гео-страницы районов
делаем). Каннибализацию двух recarga-статей пересмотреть по GSC через
4–6 недель (merge+301 — только если конкуренция сохранится).
...
- **Supabase MCP и проект сайта** `qgvfnpafbzzgnryoxnoj`:
  ...
  > 2026-08-25/26: claude.ai Supabase MCP теперь ВИДИТ проект и читает SQL,
  > но `execute_sql` работает в **read-only** транзакции — UPDATE падает.
  Актуально: чтение — через MCP, записи (закрытие рекомендаций и т.п.) —
  по-прежнему REST с service-role из `.env.local`. При этом в локальном
  `.env.local` `SUPABASE_SERVICE_ROLE_KEY` может быть пустым — реальные
  значения в Vercel (`vercel env pull`) [проверено 2026-08-10; ключ был
  на месте 2026-08-25]. В git-worktree файла `.env.local` НЕТ — сорсить
  из основного чекаута. Подробности и второй
