...
> **Противоречие (2026-08-26, вечер):** выше и в записях 25–26.08 причиной
> назывался «холодный старт shop-api на Render (free-tier сон)». Это
> неверно: shop-api с создания на платном Starter (blueprint
> `render.yaml` репо 24clima-shop), инстанс не засыпает — метрики Render
> показали 24 ч ровной работы без рестартов в день инцидента. Актуальная
> причина: **мёртвые соединения к БД** — Supavisor (session-пулер
> Supabase, проект `fbnxowigwyblwsfosata`) рвёт простоявшие соединения
> (`db_termination` when busy), а у engine'ов shop-api не было ни
> таймаутов, ни `pool_recycle`, поэтому запрос по мёртвому сокету ждал
> вечно (зависший await — не ошибка). Улика: supavisor «Terminate
> received from client» совпадает секунда в секунду с 504 Vercel
> (14:51:39 и 21:47:43 UTC). Фикс — PR #26 в 24clima-shop (таймауты
> asyncpg + statement_timeout + pool_recycle, все три engine'а; ревью
> Codex). Warm-ping как средство от «сна» не нужен; 45s-таймаут сайта
> (PR #58) остаётся второй линией обороны.
