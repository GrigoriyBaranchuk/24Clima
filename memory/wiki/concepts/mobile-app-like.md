...
6. **Токены дизайн-системы внутри globals.css** (2026-09-17, ветка
   perf-css-requests): `@import` первой строкой + `postcss-import` в
   `postcss.config.mjs`; блокирующих CSS-запросов на главной 3 → 2, разметка
   побайтно та же. Одного `@import` мало — см. грабли №40.
   **Отвергнуто:** `experimental.inlineCss` — FCP лучше на ~200 мс, но HTML
   главной 185 → 405 KB (CSS дублируется и в `<style>`, и в RSC-payload),
   суммарный трафик +65 KB на холодную загрузку, флаг «не для прода» по
   документации. Коммит эксперимента с цифрами — b72e06d (не в main).
   **Резерв на будущее:** типографика `.prose*` = 28,6 KB из 105 KB файла,
   нужна только статьям и описаниям товаров; вынос в CSS блога возможен, но
   меняет порядок каскада относительно утилит — только с визуальной сверкой
   статей.
