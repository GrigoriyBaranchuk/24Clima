...
| 31 | «Something went wrong» сразу после прод-деплоя | браузер со старым HTML/JS запрашивает чанки/RSC нового билда → `ChunkLoadError` → `error.tsx`; в Vercel runtime-логах пусто (ошибка клиентская), перезагрузка лечит — выглядит как «мистика» | это version skew: включить Skew Protection в Vercel (Next ≥14.1.4 — без доп. конфига) + error boundary сам один раз перезагружает страницу при chunk-ошибках (`src/lib/skew-error.ts`, PR #50); корневой layout ловит только `global-error.tsx` (случай 2026-08-23) |
