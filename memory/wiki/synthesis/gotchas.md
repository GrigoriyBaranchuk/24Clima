...
| 31 | «Something went wrong» сразу после прод-деплоя | браузер со старым HTML/JS запрашивает чанки/RSC нового билда → `ChunkLoadError` → `error.tsx`; в Vercel runtime-логах пусто (ошибка клиентская), перезагрузка лечит — выглядит как «мистика» | это version skew. Skew Protection в Vercel УЖЕ включена (12 ч, максимум Pro) — но не спасает вкладку старше окна: два деплоя подряд (23:51 и 23:58) вытесняют билд через минуты, и его ассеты исчезают через 12 ч. Поэтому error boundary сам один раз перезагружает страницу при chunk-ошибках (`src/lib/skew-error.ts`, PR #50) и показывает `Ref: digest` для остальных; корневой layout ловит только `global-error.tsx`. Проверка защиты: в HTML прода есть `dpl_…` (случай 2026-08-23) |
