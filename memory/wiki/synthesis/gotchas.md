...
related: [concepts/design-system-package, concepts/i18n-dual-route-tree, concepts/seo-monitoring-system, concepts/agent-workflow, concepts/panama-advertising-law, concepts/header-navigation, concepts/vercel-deploy-and-errors]
status: current
---
...
| 30 | Превью-домены Vercel закрыты Deployment Protection | `curl` по `*-git-*.vercel.app` отдаёт 302 на `vercel.com/sso-api` — выглядит как «превью сломано» или как редирект middleware, хотя деплой `pass` | это SSO, а не баг: смотреть превью в браузере под своим логином Vercel, а автоматическую проверку HTML делать на локальном `next start` того же билда (случай 2026-08-22, PR #47) |
| 31 | «Something went wrong» сразу после прод-деплоя | браузер со старым HTML/JS запрашивает чанки/RSC нового билда → `ChunkLoadError` → `error.tsx`; в Vercel runtime-логах пусто (ошибка клиентская), перезагрузка лечит — выглядит как «мистика» | это version skew. Skew Protection в Vercel УЖЕ включена (12 ч, максимум Pro) — но не спасает вкладку старше окна: два деплоя подряд (23:51 и 23:58) вытесняют билд через минуты, и его ассеты исчезают через 12 ч. Поэтому error boundary сам один раз перезагружает страницу при chunk-ошибках (`src/lib/skew-error.ts`, PR #50) и показывает `Ref: digest` для остальных; корневой layout ловит только `global-error.tsx`. Проверка защиты: в HTML прода есть `dpl_…` (случай 2026-08-23) — [деплой на Vercel](../concepts/vercel-deploy-and-errors.md) |

## Связи

- [Процесс работы агентов](../concepts/agent-workflow.md)
- [Деплой на Vercel и ошибки на клиенте](../concepts/vercel-deploy-and-errors.md) — грабли №30–31
